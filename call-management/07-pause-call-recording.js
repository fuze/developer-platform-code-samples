// ----------------------------------------------------------------------------
// PAUSE RECORDING OF AN ONGOING CALL SAMPLE CODE
// ----------------------------------------------------------------------------
// This sample code is interactive.
// It lists one user's ongoing calls and asks which one to pause the recording.
    ...
    connector.listUserOngoingCallsPromise(userId)
        ...
        .then(body => {
            ...
            if (body.data.length > 0) {
                console.log("\n\nSelect one call id to pause the recording")
                var callId = readline.question("CallId: ")
                return connector.pauseCallRecordingPromise(callId)      
            } else {
                process.exit(0)
            }
        })
